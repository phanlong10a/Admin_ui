...
class SelectAssetPopup extends React.Component {
  ...
  handleOpenSupplyDialog = () => {
    this.setState({
      shouldOpenSupplyDialog: true
    })
  }
  ...
  render() {
    ...
    let { keyword, shouldOpenSupplyDialog, itemList } = this.state;
    let columns = [
      {
      ...
      { title: t("component.Supply.code"), field: "code", align: "left", width: "250px" },
      { title: t("component.Supply.name"), field: "name", align: "left", width: "150" },
    ];
    return (
      <Dialog onClose={handleClose} open={open} PaperComponent={PaperComponent} maxWidth={'md'} fullWidth>
        <DialogTitle style={{ cursor: 'move' }} id="draggable-dialog-title">
          <span className="mb-20">{t("component.Supply.title")}</span>
        </DialogTitle>
        <DialogContent style={{height:'370px'}}>
          ...
          <Grid item xs={12}>
            <div>
              {shouldOpenSupplyDialog && (
                <SupplierDialog t={t} i18n={i18n}
                  handleClose={this.handleDialogSupplyClose}
                  open={shouldOpenSupplyDialog}
